...
| V2 | G-62/M-27 delta (a) (R4: **RESOLVED**) | — | The R2 delta above ("chrome hanging off the pill's bottom edge, not the pill *becoming* a surface") was judged in R4 as reading like **two shapes** — a card floating ~4pt under a pill that kept its own amber ring, against §B's "the single black shape begins to grow… the edge-light stretches continuously". Fixed without touching `morphProgress` (still the one geometry owner; the peek still owns no morph state and is still non-interactive chrome): the peek is now the pill's **own column** — `dockedWidth` = `closedPillOuterWidth()` (§B′'s filmstrip draws the peek frame at the pill's width, `132×58` after `132×30`, floored at the board's 408 so a short pill still has room to narrate), its top band is transparent so the live pill draws into it, and the fill is `HaloHoverPeekDockShape`: the whole column **minus the pill's silhouette**, even-odd. The subtraction is what makes it one body — a square-top body butted under a `height/2`-radius pill leaves two corner notches at the joint, and butting the two antialiased edges left a **measured 1px α≈198 hairline** straight across it (bright on any light desktop), so the knockout is pulled 0.75pt *inside* the pill's silhouette and the ink underlaps. Lighting unified the other way the brief allowed: the docked column carries **one** continuous outline (the same `theme.surfaceEdgeOverlay` seam, handed the union's shape and its measured size) and the pill's own ring stands down for as long as the peek is up (`suppressesClosedEdgeForPeek`, gated in both the morph and the Reduce-Motion path). The peek also moved **inside** the 1.03 hover scale — outside it, a scaled pill over an unscaled column stepped the joint by ~14pt, the very tell being fixed. Verified live (`CGWarpMouseCursorPosition` + `CGEvent` ⌥⌘O): `after-R4-peek-hover.png` (one body, one amber outline, zero translucent rows through the joint at x=600), `after-R4-peek-unhover.png` (pill's own ring restored, no residue), `after-R4-peek-morph-handoff.png` (⌥⌘O opens clean). Pinned by `HaloHoverPeekDockTests` |
| V2 | I′ notch collision (R4: **RESOLVED — was BLOCKING**) | — | The judge measured Halo's §I′ pill rendering its only critical signal **under the hardware cutout**: bright content at x 776.5…847.5 against this MacBook's 663.5…848.5 `auxiliaryTop*Area` cutout — **61.0pt eaten by the notch** (`judge-R-02-closedCritical-notch-collision.png`; reproduced before the fix and measured from the window capture's alpha+luminance, not eyeballed). Root cause was a *reservation* drift, not a paint bug: `V6RightSlotView.intrinsicWidth(of: .usage)` returns the `×N` **badge's** ~22pt, and Halo drew a ~145pt `arc + Codex 92% + 18h 59m` group into it — right-aligned to the pill edge, so the overflow ran backwards into the cutout. The board never asked for one group: `06-halo.html:1301-1305` puts the arc + `Codex 94%` on the **left** wing and the countdown **alone** on the right. Split accordingly — `HaloUsageFilamentLead` (left) + `HaloUsageCountdown` (right), with `HaloClosedPillWings` as the one pure rule both the pill and the layout read, surfaced through a new defaulted `IslandTheme.closedPillWingPlan(label:rightSlot:height:)` and a defaulted `leadingAccessoryWidth:` on `V6ClosedPill.{macbook,external}OuterWidth`. Every other theme and every other Halo state take the identity plan (accessory 0, label untouched), so their widths are byte-identical (existing goldens unchanged). The plan also drops the lane label for `.usage` — the left wing already reads `Codex`, so the resolver's own `Codex` label printed the vendor twice. Verified numerically after: pill 514.0…998.0, left-wing content ends **656.0** (7.5pt clear of the cutout), right-wing countdown 930.0…975.0 — **0.0pt total overlap**, and `closed` / `closedAttention` re-measured at 0.0pt too (`after-R4-{closedCritical,closed,closedAttention}.png`). Pinned by `HaloClosedPillWingSplitTests.usageLeadEndsBeforeThePhysicalCutout` |
| V2 | Working-pill narration (R4: **assessed — SKIPPED, needs shared logic**) | Working pill's centre label reads `Running sed -n '…` where §A wants `Editing AppModel.swift` | Not doable Halo-side without new plumbing, so parked per the brief's own escape clause. The narration helper already exists and is already wired — `ActivityNarrator.narrate(toolName:preview:)` → `AgentSession.narratedActivity` → `IslandClosedLabelResolver.runningLabel` → the pill's label. It is producing the *correct* answer for the input it gets: the `runningSession` fixture's tool is `exec_command` with preview `sed -n '1,260p' Sources/…/SettingsView.swift`, and `exec_command`/`bash` maps to `.running` + `commandObject`, i.e. "Running <command>". Rendering it as `Reading SettingsView.swift` means teaching `ActivityNarrator` a **shell-command → intent lexicon** (`sed -n` reads, `sed -i` edits, and every sibling), which is theme-agnostic code consumed by all six themes' rows, the closed-label resolver, Flight Deck's narration segments and the §B peek — a shared behaviour change, not a Halo restyle. Deriving it Halo-side instead is also blocked: `theme.closedPill(...)` receives an already-resolved `String`, never the session, so a Halo-only verb+filename would need a new session seam through the closed-pill factory. Both routes exceed the brief's "no new shared logic / ~an hour" bound; items 1 and 2 were the priority. Owner call whether the lexicon is worth it — it would improve every theme at once |
