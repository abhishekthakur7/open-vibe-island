...
| V4 | G-10 | Active-row rail still not readable — the 2pt rail sits in the same left-gutter column as the panel's animated perimeter edge-light, which dominates it | Not a row bug (rail renders, verified in code + 20× zoom); it's a perimeter/V0 interaction — dimming the perimeter further is an owner call |
...
| V5 | G-57 (partial) | No `Reply` affordance on `completionCard` / `longCompletionCard` / `completedInterrupted` / `completedFailed`; `Dismiss` is absent there too | Those four scenarios render the **notification card**, whose `notificationRowActions` (`IslandPanelView.swift:1139-1147`) deliberately carries no `dismiss`, and whose `reply` is gated on `TerminalTextSender.canReply` — false for every fixture. Adding either to the notification card is a behaviour change, not a Halo visual fix. The §H list row does show both correctly (see `after-V5-sessionList-completedRow.png`) |
| V5 | G-49 (residual) | The done-row line is the message's first sentence with markdown rendered away — not a true narration ("Updated `AGENTS.md` & `CLAUDE.md` with …") | A real narration needs a summarising pass over the transcript that no existing field provides; the row has no model to call. `HaloActivityNarration` is the honest minimum: no raw markdown source, no ellipsis-truncated dump. Upgrading it means new data plumbing — owner call |
...
| V4 | — (cosmetic) | On a running row the trailing age and the elapsed clock chip can render the same string (`2m` / `🕐 2m`), reading as a duplicate; chip label ink sits one step hotter than the mockup's chip tier | Value coincidence between two semantically different slots, flagged by the judge as non-blocking — suppress-when-equal or finer clock units (`1m 42s`) if the owner wants it |
| V7 | G-21 (verify-blocked on `approvalCard`) | Scoped-grant rows can never render on `approvalCard` — its fixture (`IslandDebugScenario.swift:543`) builds a `PermissionRequest` with no `toolName` and no `suggestedUpdates`, so both row branches are structurally unreachable | Fixture edit = state-driving infrastructure, not sanctioned. Row proven end-to-end on `diffApprovalCard` (judge PASS incl. M-19 hover). One-line truthful fix if wanted: `toolName: "exec_command"` on that fixture (`claudeMetadata.currentTool` already says it) |
| V7 | G-21 (cosmetic) | Scope label tail-truncates at 520pt: `Yes, allow writing to AGENTS.md from t…` — trailing "from this project" cut | Root cause is `ClaudePermissionUpdate.displayLabel` in Core being wordier than the mockup copy (`writing to … from this project` vs `edits to … in this project`); shortening it is a Core copy change outside Halo scope |
| V7 | G-22/G-48 (cosmetic) | With the row's activity line suppressed during a hero, the `.lead` column's second slot keeps its height — the agent badge sits alone on an empty line, leaving dead space between row title and hero card | Judge non-blocking note; collapsing the row to one line while a hero is presented is a small layout follow-up, owner call |
...
| V7 | — (cosmetic) | Codex CTA lacks the mockup's `⌘J` keycap; its `who` chip renders a bare `C` with no model label (mockup: `X Codex`) | Judge non-blocking note — content/affordance additions beyond the G-73 hue fix, owner call |
| V8 | G-04 (verify-later) | `lineLimit(1)` → `lineLimit(2)` landed, but no fixture description is long enough to wrap at the panel's width — the two-line `.od` the board shows was never rendered live | Every `questionCard` / `multiQuestionCard` option description is one short sentence; a longer one needs a fixture edit (state-driving infrastructure, not sanctioned). Structurally verified in code |
| V8 | — (cosmetic, V7 family) | On `questionCard` the collapsed row's activity line ("Which authentication method should we use") repeats the hero's question sentence verbatim — the same duplication G-22/G-48 fixed for the **permission** hero | The suppression seam (`HaloSessionRow.swift`, `permissionHeroNarration`) is V7-owned and only covers `edgeState == .permission`; extending it to `.question` edits a locked view. One-line follow-up if the owner wants it |
| V9 (final gate) | V1 residual (Q2: **re-measured, still parked**) | Right-of-notch header filament never renders on this MacBook. Q2 instrumented `HaloHeaderControls` on the live overlay: surface 540pt → `rawRightWidth` 131.5, minus `notchLaneSafetyInset` 12, minus the three 26pt controls (94) + 8pt gap = **17.5pt** for the gauge — not the ~52 this row assumed, and `rightUsageWidth` snaps to 0 well before the 58 floor is consulted | The proposed fix (lower `minimumRightUsageLaneWidth`) cannot work: no floor admits a gauge into 17.5pt, and Halo's own §C filament needs **more** than 58 (30pt arc + 9 gap + 11pt percent ≈ 64; ≈56 even at the 22pt arc with the kicker dropped). Freeing the missing ~35–45pt means changing the header itself — `notchHeaderHorizontalPadding` 46 → the board's own 16 recovers exactly 30pt (→ 47.5pt lane) but shifts the three controls 30pt outward, a visible change to the locked V1 header; or shrink the mockup-spec 26pt controls; or column-stack the lane (needs a taller `openedHeaderHeight`). All three are owner calls. Measurement recorded in the `minimumRightUsageLaneWidth` doc comment so it isn't re-derived |
| V9 (final gate) | — (cosmetic, G-22/G-48 family) | `codexApprovalCard` hero subtitle duplicates the command block: `Codex wants to run: git push origin main` directly above `$ git push origin main`; mockup 06-halo.html:1049 reads `Codex needs a decision — in-app only` | Copy swap on the Codex hero subtitle — small follow-up, owner call |
