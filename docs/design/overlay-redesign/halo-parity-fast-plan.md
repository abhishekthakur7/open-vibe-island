...
| V9 (final gate) | V1 residual | Right-of-notch header filament never renders on this MacBook — `IslandHeaderLaneLayout.notchAwareMetrics` zeroes `rightUsageWidth` because the right lane's ~52pt (after notch inset + three 26pt controls) is under the shared `minimumRightUsageLaneWidth = 58` (`IslandUsageSummary.swift:215`) | `HaloHeaderControls.laneFilaments` is correct; the constant is shared by all six themes — same shared-layout blocker class as V2 G-20. Owner call to lower/stack |
...
| V9 (final gate) | — (copy) | Group headers read `IN PROGRESS` / `JUST DONE` / `IDLE`; mockup §C says `NEEDS YOU` / `RUNNING` / `DONE` | Shared localized cross-theme strings (`island.section.*`, `Localizable.strings:473-478`) — product taxonomy call, not a Halo skin bug; composition matches |
...
| V9 (final gate) | M-26 residual | Morph envelopes slightly asymmetric: open ≈0.19s, close ≈0.13s (120fps capture). Structural complaint fixed — close is a true reverse-geometry shrink | Non-blocking by eye; equalize durations if the owner wants exact symmetry |
