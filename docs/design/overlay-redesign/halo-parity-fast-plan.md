...
| V9 (final gate) | — (test infra, **pre-existing**) | `swift test` reports **64** `Missing snapshot golden` errors from `HaloConformanceSnapshotTests` / `PouredConformanceSnapshotTests` / `ThemeSnapshotHarnessTests` — the `__Snapshots__/` directories carry only `environment-fingerprint.txt`, and this machine's fingerprint *matches*, so the pixel compare runs instead of skipping and every golden is missing | Not caused by any parity batch: verified identical (64, and nothing else) by stashing the whole R2 diff and re-running the full suite at `088e64cd`. Either record them once with `OPEN_ISLAND_RECORD_SNAPSHOTS=1` or drop the stale fingerprint file so the gate skips — owner call, and either way it is a test-infra decision, not a Halo look |
